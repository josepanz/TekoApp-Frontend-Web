...
#### Estado real de las 4 specs y de qué depende cada una (actualizado 2026-09-06)

Las 4 specs ya están escritas (ver §8). Lo que las bloqueaba se consolidó en un solo pedido al
backend: `TekoApp-Backend/openspec/changes/0015-admin-backoffice-endpoints.md`. Mapeo exacto:

| Spec de Web                                | Depende de                                                          | Se puede implementar                                                                                                                                                                    |
| ------------------------------------------ | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `openspec/specs/admin-audit-log-viewer.md` | `W-01` de `0015` (`GET /admin/audit-logs`)                          | cuando `W-01` esté mergeado                                                                                                                                                             |
| `openspec/specs/admin-data-export.md`      | `W-02` de `0015` (`GET /admin/payments/export`, ídem professionals) | cuando `W-02` esté mergeado. Alcance de la 1ª iteración de backend: **CSV solamente**, no `.xlsx`                                                                                       |
| `openspec/specs/admin-global-search.md`    | `W-03` de `0015` (filtro `search` en los listados)                  | cuando `W-03` esté mergeado. **No** habrá endpoint agregador `GET /admin/search`: la agregación la hace Web pegándole en paralelo a `GET /users?search=` y `GET /professionals?search=` |
| `openspec/specs/admin-bulk-actions.md`     | **nada**                                                            | **ya** — usa los endpoints de un solo ítem que ya existen (`DELETE /categories/:id`, `DELETE /ratings/:id`, …) llamándolos N veces desde el cliente                                     |

Corrección de una nota vieja del §8: decía "las 3 primeras dependen de un endpoint nuevo de
backend". Es incorrecto por el orden en que se commitearon las specs — la que **no** depende de
nada es `admin-bulk-actions.md`, y es por lo tanto la única de las 4 que se puede arrancar sin
esperar al backend.

**Cuando cualquier `W-0N` se mergee**, el primer paso del lado Web es correr
`pnpm generate:api-types` (prohibido escribir esos tipos a mano) y recién después implementar.

#### Estado de implementación (actualizado 2026-09-07)

| Spec                                       | Estado                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `openspec/specs/admin-bulk-actions.md`     | **Implementada (piloto)** — commit `e4c84be` en `audit/2026-09-04`. `DataTable` ganó selección de filas controlada (`selection` prop: columna de checkbox + "seleccionar todo" en la página actual), `BulkActionsBar` (nuevo, `components/layout/`) y `runBulkAction` (nuevo, `src/lib/`, concurrencia limitada + reporte de éxito/fallo parcial, nunca un binario que oculte fallos). Piloto en `categories-table.tsx`: seleccionar → "Eliminar seleccionadas (N)" → confirmar → toast distingue éxito total/fallo total/parcial con nombres. **Pendiente** (fuera de esta sesión, la spec ya lo marca como "tareas separadas"): extender el mismo patrón a `ratings-table`, `professional-portfolio` y `professionals` — los tres tienen la barra/hook genéricos listos para reusar, solo falta cablear la acción de cada tabla.          |
| `openspec/specs/admin-audit-log-viewer.md` | **Implementada** — commit `c3259cf` en `audit/2026-09-04`. `/admin/audit-log` (solo lectura) contra `GET /admin/audit-logs` (backend, `W-01` de `0015`, ya tipado). Tabla paginada server-side con filtros por tabla, registro, quién y rango de fechas + diálogo de detalle con el diff `oldData`/`newData` como JSON. Permiso nuevo `SYSTEM.AUDIT_VIEW` (`system.audit:read`) agregado a mano en `permissions.ts` (sin codegen para permisos), gate igual a `admin:all`. Desvío respecto a la spec: los filtros son inputs de texto (match exacto, así documenta el backend), no el "select" que sugería la spec — no existe ningún endpoint que exponga el catálogo de tablas auditadas (el trigger genérico del backend decide qué tabla califica), así que un select hubiera sido una lista inventada y potencialmente desactualizada. |
| `openspec/specs/admin-data-export.md`      | **Implementada** — commit `da4b2cf` en `audit/2026-09-04`. Botón "Exportar" (CSV, `lucide-react` `Download`) en `payments-table.tsx` (respeta el filtro de estado activo) y `professionals-table.tsx`, contra `GET /admin/payments/export`/`GET /admin/professionals/export` ya publicados por el backend (`0015-admin-backoffice-endpoints.md` `W-02`). El backend responde el CSV vía `StreamableFile` + `Content-Disposition`, sin el envelope `{success,data}` habitual — se agregó `downloadFile()` en `core/api-client/client.ts` (`.blob()` + filename desde el header) y `triggerFileDownload()` en `src/lib/` para la descarga nativa. Mismo permiso que gatea cada tabla (`PAYMENTS.AUDIT_VIEW`/`PROFESSIONALS.VERIFY`, `admin:all`) — sin permiso nuevo.                                                                         |
| `openspec/specs/admin-global-search.md`    | **Implementada (opción federada)** — commit `076c1ac` en `audit/2026-09-04`. Ícono de búsqueda en el topbar de admin (Ctrl/Cmd+K), `features/global-search/`, agrega client-side `GET /professionals?search=` + `GET /users?name=` con debounce de 300ms. **Corrección sobre este renglón**: `GET /professionals` sí tiene `search` (parcial, insensible a mayúsculas, ya documentado), pero `GET /users` **nunca tuvo** un `search` unificado — tiene `name` (contains + insensitive sobre firstName/lastName, verificado en `ListUsersRequestDTO`/`UsersDBService.findAllUsers` del backend), que no matchea por email/documentNumber. Se usó `name` en vez de inventar un `search` inexistente en `/users`. Un 403/500 en una de las dos búsquedas no oculta los resultados de la otra.                                                  |

**Hallazgo técnico no anticipado por la auditoría original**, encontrado implementando la
selección de filas: `flexRender` (TanStack Table) trata cualquier función pasada como `cell`/
`header` de una columna como si fuera un componente React (`React.createElement(fn, props)`).
Si esa función se recrea con una referencia nueva en cada render — como pasaba con la columna
de selección, que dependía de un objeto `selection` con identidad inestable —, React desmonta y
remonta el control en cuanto el padre vuelve a renderizar, perdiendo el estado/foco del que se
acaba de tocar (se manifestaba como "solo se puede seleccionar una fila por vez": la segunda
selección no pegaba). Se corrigió memoizando la columna de selección con deps vacías
(`src/components/layout/data-table.tsx`). **Relevante para cualquier columna nueva que dependa
de un objeto/función recreado en cada render** — no es exclusivo de la selección.
...
**Actualizado 2026-09-06**: la definición ya existe —
`TekoApp-Backend/openspec/changes/platform-hardening-2026-09/I-01-account-deletion.md` (anonimizar
solo `Users`, el resto hereda por FK; ventana de 14 días; bloqueantes explícitos por cruce con
disputas). Lo que sigue faltando es la **implementación** del endpoint, no la especificación, así
que esta tarea sigue bloqueada. Mobile tiene su propia spec del mismo flujo
(`TekoApp-Frontend-Mobile/openspec/specs/account-deletion.md`, commit `274f947`) contra ese mismo
contrato: **coordinar la copia de los avisos legales con ella**, no redactarla de nuevo.
...
`[~]` = parcialmente hecha (algunas sub-tareas cerradas, otras siguen pendientes — ver notas y la
sección de la tarea para el detalle).
...
| G-03 | MEDIO   | [x]    | 531f1d4                                                                                           | Card de preview (avatar iniciales, rating, badges) + barra de completitud + Card en el form. Verificado claro y oscuro con screenshot real (professionals/me mockeado en el QA, el fake-backend no lo implementa)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| G-04 | MEDIO   | [x]    | b4aa593                                                                                           | Ícono+color ya estaban (fix previo); agregado Badge `success`/`secondary` junto al Switch — el estado ya no depende solo de la posición del switch. Verificado claro y oscuro                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| G-05 | BAJO    | [x]    | c6d6a2a                                                                                           | Hecha igual por pedido explícito de José pese a "Opcional". Permisos agrupados por dominio, badge Activo/Inactivo con variant `success`/`secondary`, fecha de creación agregada. Verificado claro y oscuro                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| G-06 | MEDIO   | [x]    | 5f55f17, 5a5dbb5                                                                                  | Cerrado 2026-09-15. `notifications-table` y la cola de `professional-portfolio` ya mostraban nombres (verificado en `5f55f17`); solo `ratings-table` quedaba con el parche de `#null` → "Anónimo". El backend agregó `userName`/`professionalName` a `RatingDetailResponseDTO` (tarea 9, mismo criterio de anonimato que los ids) — `5a5dbb5` los consume. Matiz importante: `isAnonymous` decide la etiqueta, no la nulidad del nombre — para admin/staff el backend manda el nombre real incluso en una calificación anónima (tiene permiso de auditoría), pero la UI sigue mostrando "Anónimo" para respetar la elección del usuario; solo se muestra el nombre real cuando `isAnonymous` es `false`. Fallback defensivo a `#id`/'—' si el nombre viniera null sin ser anónima (no debería pasar hoy)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| I-01 | MEDIO   | [x]    | df0adc3, aeefea6, 45f5151, a6e88c7, e4c84be, da4b2cf, c3259cf, 076c1ac, fa9bc28, d4b62a0, dc13265 | 4 specs escritas y las 4 implementadas. `admin-bulk-actions.md` (piloto en `categories-table`, commit `e4c84be`, extendido a las 3 tablas restantes 2026-09-12: `ratings-table` con eliminar en bloque, commit `fa9bc28`; cola de `professional-portfolio` con aprobar/rechazar en bloque — rechazo pide un motivo compartido, obligatorio en el DTO —, commit `d4b62a0`; `professionals` con verificar/suspender en bloque sobre los endpoints de un solo ítem ya existentes, commit `dc13265`. Las 3 reusan `BulkActionsBar`/`runBulkAction` sin tocar la infra genérica, y el detalle de fallo parcial ahora usa `ApiError.message` real por ítem en vez de un genérico). `admin-data-export.md` (commit `da4b2cf`). `admin-audit-log-viewer.md` (commit `c3259cf`). `admin-global-search.md` (commit `076c1ac`, opción federada, con la corrección de `/users` sin `search` documentada ahí). Ver detalle de cada una en §6                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| I-02 | MEDIO   | [x]    | 30dd544                                                                                           | Ya no bloqueada — backend implementado en `a24cf7f` (`TekoApp-Backend`). Sección nueva en `/perfil` (`features/account-deletion/`): confirmación de dos pasos + banner persistente durante la ventana de gracia + listado de los 4 bloqueantes reales en un 409. Endpoints reales `POST /auth/me/deletion-request`/`.../cancel` (no `/users/me/...` como decía la spec original de backend antes de su propia desviación documentada). Copia alineada contra `TekoApp-Frontend-Mobile/openspec/specs/account-deletion.md` (esa spec sigue sin implementación, así que se alineó contra los mismos hechos técnicos, no contra strings ya escritos). Hallazgo aparte que en su momento quedó documentado sin corregir (código compartido, fuera de alcance de esta tarea) — **ya corregido, commit `65a7ac7`**: `parseErrorResponse` de `client.ts` no desenvolvía `{success:false,error:{message,...}}`, así que `error.message` caía al fallback genérico para todas las features en producción. Se agregó `extractErrorInfo` (soporta el envelope real y el DTO pelado de MSW/fake-backend) y `errorCode`/`details` tipados en `ApiError`; `extractDeletionBlockers` ya no cava en `error.body` a mano, lee `error.errorCode`/`error.details`. Los bloqueantes de esta pantalla nunca dependieron del bug (ya leían `error.body` directo, sin pasar por `message`), así que ya se mostraban bien antes del fix — lo que sí caía al genérico era cualquier pantalla que mostrara `error.message` tal cual. |
| I-03 | BAJO    | [x]    | 9da190f, 3662b2e, cb68a2d, 9de66d6                                                                | Doc corregido en `9da190f`. Backlog de e2e cerrado 2026-09-12: pagos (`3662b2e` — reembolso/cancelación/propina, `e2e/payments.spec.ts`), postulación→verificación (`cb68a2d` — `e2e/professional-verification-funnel.spec.ts`, misma cuenta cambia de modo porque el fake-backend solo tiene un usuario), colas de revisión (`9de66d6` — documentos y portafolio, `e2e/review-queues.spec.ts`). De paso salieron dos bugs de la propia suite e2e (no de estas 3 tareas): `playwright.config.ts` no fijaba `locale`, así que en un runner en inglés la app entera se servía en inglés y ningún spec podía loguearse; y `getByLabel('Contraseña')` sin `exact:true` matcheaba también el botón de mostrar/ocultar contraseña (`aria-label="Mostrar contraseña"` contiene la palabra). Corregidos en `3662b2e` junto con el primer flujo, porque bloqueaban toda la suite, incluidos los specs preexistentes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
